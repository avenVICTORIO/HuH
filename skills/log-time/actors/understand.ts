...
// Also recognises when the person wants to cancel.
...
const actor: Actor = {
  ...
  name: "Verstehen",
  kind: "ai",
  description: "Liest Datum, Beginn und Ende aus der Nachricht; fragt nach, was fehlt; erkennt einen Abbruchwunsch.",
  pos: { x: 40, y: 120 },
  output: {
    type: "object",
    properties: {
  ...
  async handle(message, ctx) {
    ...
    const system = [
      `Du extrahierst Arbeitszeiten aus Chat-Nachrichten eines Restaurant-Teams. Heute ist ${WD[today.getDay()]}, der ${ctx.today}.`,
    ...
    if (missing.length) {
      ...
      return { ask: question, state: { time, correction: null } };
    }
