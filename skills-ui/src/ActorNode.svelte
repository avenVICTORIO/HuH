...
<div class="actor {data.kind} {data.ref ? 'ref' : ''} {data.active ? 'active ' + (data.status ?? '') : ''}">
  ...
  {#if data.inKeys?.length || data.outKeys?.length}
    <div class="io">
      {#if data.inKeys?.length}<span><i>in</i>{data.inKeys.join(", ")}</span>{/if}
      {#if data.outKeys?.length}<span><i>out</i>{data.outKeys.join(", ")}</span>{/if}
    </div>
  {/if}
  {#if data.loop}<div class="loop">↺ {data.loop}</div>{/if}
...
<style>
  ...
  .kind { font-size: 10px; letter-spacing: .12em; text-transform: uppercase; font-weight: 600; color: #6B6255; }
  .ai .kind { color: #B0553A; }
  ...
  .pulse { margin-left: auto; width: 10px; height: 10px; border-radius: 50%; background: #B0553A; box-shadow: 0 0 0 0 rgba(176,85,58,.5); animation: pulse 1.4s infinite; }
  @keyframes pulse { 70% { box-shadow: 0 0 0 8px rgba(176,85,58,0); } 100% { box-shadow: 0 0 0 0 rgba(176,85,58,0); } }
  ...
  .io { margin-top: 8px; display: flex; flex-direction: column; gap: 2px; font-size: 10.5px; color: #2B2A26; font-family: ui-monospace, Menlo, Consolas, monospace; }
  .io i { font-style: normal; font-family: Montserrat, sans-serif; font-size: 9.5px; letter-spacing: .1em; text-transform: uppercase; color: #8A7F6E; margin-right: 6px; }
  .loop { margin-top: 8px; font-size: 10.5px; color: #8A7F6E; background: #F1EBDF; border-radius: 8px; padding: 3px 8px; display: inline-block; }
